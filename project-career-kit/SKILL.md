...
- 未满足复杂图条件时，`项目流程图.md` 的“可选复杂流程”只保留标题和非项目事实的占位注释，不枚举“未确认多服务/事务/鉴权”等无证据范围否定。为避免 Mermaid 解析歧义，节点标签使用简洁中文，内部 ID 使用描述性名称而不显示为用户标签；只有被证据直接支持且已验证 Mermaid 语法安全时，才放入函数名、调用参数、状态码和错误文本。可见文字只允许按需提供一条简短边界说明，且只说明有证据支持的运行时未知项或 Mermaid 兼容性限制；不得写节点/边的长篇图例。
